...
{	
	...
	if(verify_input_text('/[^0-9]/', $first_id))
	{
		...
		header('Location: '.$BASE_URL.'pages/set_bo1.php');
		exit();
	}
}
else
...
{
	$_SESSION['state']="1";
	$_SESSION['message']="Error ! Last ID is not filled /!\ Operation aborted /!\ ";
	header('Location: '.$BASE_URL.'pages/set_bo1.php');
	exit();
}
...
if(isset($_POST['overtime_money']) && !empty($_POST['overtime_money']))
{	
...
{
	$_SESSION['state']="1";
	$_SESSION['message']="Error ! Ovetime money is not filled /!\ Operation aborted /!\ ";
	header('Location: '.$BASE_URL.'pages/set_bo1.php');
	exit();
}
...
if(!empty($first_id) and !empty($last_id) and !empty($rules) and !empty($overtime_money))
{
	if($first_id>$last_id)
	{
		$_SESSION['state']="1";
	...
	$check_status = $BDD_EBOT->prepare('SELECT id, status, enable FROM matchs WHERE id BETWEEN ? AND ?');
	...
	$req1 = $BDD_EBOT->prepare("UPDATE matchs SET rules= ?, config_password= ?, max_round = ?, season_id = ?, config_ot= ?, config_knife_round= ?, overtime_max_round= ?, overtime_startmoney= ? WHERE status=0 AND enable IS NULL AND id BETWEEN ? AND ?");
	$req1->execute(array($rules, $ebot_pass, $match_mmr, $season, $overtime_status, $knife, $overtime_mmr, $overtime_money, $first_id, $last_id));
	$req1->closeCursor();
	
	$req2 = $BDD_EBOT->prepare("UPDATE maps SET map_name= ? WHERE status=0 AND id BETWEEN ? AND ?");
	$req2->execute(array($map, $first_id, $last_id));
	$req2->closeCursor();
	
	$_SESSION['state']="2";
	$_SESSION['message']="Map ".$map." has been updated on matches #".$first_id." to #".$last_id;
	$ip = get_ip();
